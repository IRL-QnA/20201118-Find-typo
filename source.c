...
/*상수 정의*/
#define BLOCK_SIZE  8 //DES 블록 사이즈 수
...
UINT f(UINT in, BYTE* rkey);                                //f함수
void key_expansion(BYTE *key, BYTE exp_key[16][6]);        //키 확장 함수
void swap(UINT* x, UINT* y);                               //스왑(자리바꿈) 함수
...
void main() //int main(void)로 바꿔야 할 수 도 있다.
{
	int i;
	int msg_len = 0, block_count = 0;          //메세지 길이와 블록 수
...
void DES_Encryption(BYTE *p_text, BYTE *result, BYTE *key) 
{
	...
	WtoB(L, R, data);             //32bit로 나누어진 블록을 다시 64bit블록으로 변환
